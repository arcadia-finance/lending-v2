...
/**
 * @notice Fuzz tests for the function "_settleAuction" of contract "LiquidatorL1".
...
contract SettleAuction_LiquidatorL1_Fuzz_Test is LiquidatorL1_Fuzz_Test {
    ...
    function testFuzz_Success_settleAuction_AfterCutoff(
        ...
        bool success = liquidator_.settleAuction(address(account));
    ...
    function testFuzz_Success_settleAuction_NoRemainingValue(uint112 usedMargin, uint96 minimumMargin) public {
        ...
        registry_.setRiskParameters(address(pool), type(uint128).max, type(uint64).max);
        ...
        public
    {
        ...
        registry_.setRiskParametersOfPrimaryAsset(address(pool), address(mockERC20.stable1), 0, type(uint112).max, 0, 0);
        ...
        assertTrue(liquidator_.getAuctionIsActive(address(account)));
    }
    ...
    function testFuzz_Success_settleAuction_NoRemainingAssets(address bidder, uint112 amountLoaned) public {
        ...
        uint256[] memory bidAssetAmounts = liquidator_.getAuctionAssetAmounts(address(account));
        deal(address(mockERC20.stable1), bidder, type(uint128).max);
        vm.startPrank(bidder);
        mockERC20.stable1.approve(address(pool), type(uint256).max);
        liquidator_.bid(address(account), bidAssetAmounts, false, "");
        vm.stopPrank();
    ...
    function testFuzz_Success_settleAuction_ZeroCollateral(
        ...
        liquidator_.setInAuction(address(account), account.creditor(), startDebt);

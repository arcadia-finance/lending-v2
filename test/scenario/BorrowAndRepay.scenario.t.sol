...
contract BorrowAndRepay_Scenario_Test is Scenario_Lending_Test {
    ...
    function testScenario_Success_borrow_IncreaseOfDebtPerBlock(
        ...
        uint128 exponent = (uint128(deltaTimestamp) * 1e18) / uint128(pool.YEARLY_SECONDS_());
        ...
        uint128 expectedDebt = uint128(
            (
                debtAtStart
                    * (
                        LogExpMath.pow(
                            _yearlyInterestRate + 10 ** 18, (uint256(deltaTimestamp) * 10 ** 18) / pool.YEARLY_SECONDS_()
                        )
                    )
            ) / 10 ** 18
        );
    ...
    function testScenario_Success_syncInterests_IncreaseBalanceDebtContract(
        ...
        uint128 exponent = uint128((uint128(deltaTimestamp) * 10 ** 18) / pool.YEARLY_SECONDS_());
    ...
    function testScenario_Success_repay_PartialDebt(
        ...
        exponent = uint128((uint128(deltaTimestamp) * 10 ** 18) / pool.YEARLY_SECONDS_());

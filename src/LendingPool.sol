...
contract LendingPool is LendingPoolGuardian, Creditor, DebtToken, ILendingPool {
     ...
     * @param account The address of the Arcadia Account backing the debt.
     * @param actionMulticall The address of the Action Multicall to call.
    ...
    function flashAction(
        ...
        // Send Borrowed funds to the actionMulticall.
        asset.safeTransfer(actionMulticall, amountBorrowed);

        // The Action Multicall will use the borrowed funds (optionally with additional assets withdrawn from the Account)
        // to execute one or more actions (swap, deposit, mint...).
        // Next the action Multicall will deposit any of the remaining funds or any of the recipient token
        ...
        // The Collateral Value of all assets in the Account is bigger than the total liabilities against the Account (including the debt taken during this function).
        {
            (address creditor, uint256 accountVersion) =
                IAccount(account).accountManagementAction(actionMulticall, actionData, signature);
            if (creditor != address(this) || !isValidVersion[accountVersion]) revert Reverted();
        }

        emit Borrow(
            account, msg.sender, actionMulticall, amountBorrowed, amountBorrowedWithFee - amountBorrowed, referrer
        );
    }
